...
detect_rhel_7 ( ) {
  ...
  if egrep ' 7\.' /etc/redhat-release &> /dev/null; then
    yum install -y https://yum.puppetlabs.com/puppetlabs-release-pc1-el-7.noarch.rpm || true
...
detect_rhel_6 ( ) {
  ...
  if egrep ' 6\.' /etc/redhat-release &> /dev/null; then
    yum install -y https://yum.puppetlabs.com/puppetlabs-release-pc1-el-6.noarch.rpm || true
    yum install -y "puppet-agent-${PUPPET_AGENT_VERSION}"
  fi
